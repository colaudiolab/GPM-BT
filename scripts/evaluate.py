...
from beans.ast_models import ASTModel
...
def eval_pytorch_model(model, dataloader, metric_factory, device, desc, loss_func):
    model.eval()
    total_loss = 0.
    steps = 0
    metric = metric_factory()
...
def train_pytorch_model(
    ...
    for lr in lrs:
        ...
        for epoch in range(args.epochs):
            ...
            test_loss, test_metric = eval_pytorch_model(
                model=model,
            ...
            if test_metric > test_metric_best:
                test_metric_best = test_metric
    ...
    return best_model, valid_metric_best, test_metric_best
...
def main():
    ...
    if args.model_type in {'lr', 'svm', 'decisiontree', 'gbdt', 'xgboost'}:
        ...
        model, valid_metric_best, test_metric_best = train_pytorch_model(
            args=args,
            dataloader_train=dataloader_train,
            dataloader_valid=dataloader_valid,
            ...
            device=device,
            log_file=log_file,
            input_tdim=dataset_train.input_tdim,
            loss_func = loss_func)
    ...
    print(
        ...
        'test_metric_best= ', test_metric_best,
        file=log_file)
